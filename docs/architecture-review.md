...
AmazingData Adapter + Historical Warehouse + Analysis Engines
    |
Redis real-time state / Historical Parquet + DuckDB / AmazingData SDK
```

整体方向是正确的。`MarketDataService`、`LimitUpService`、`TechnicalAnalysisService` 和 response mappers 已经开始承接 Router 中的数据访问与响应编排；`engines/technical`、`engines/fundamental`、`engines/factor` 基本保持纯计算，`historical/warehouse.py` 也已经把 DuckDB 查询、文件布局、同步状态封装成较深的模块。测试也已经覆盖了历史数据仓、市场数据、技术指标、基本面、因子分析、涨停榜 service 和技术分析 service，当前回归状态健康。
...
`/market/kline` 与 `/historical/kline` 已经收口到 service 层，但技术、基本面、因子路由仍需要继续服务化。当前数据路径应保持简单：历史行情和元数据优先读 Historical Parquet + DuckDB，未同步或实时查询回源 SDK；Redis 只保存实时/订阅行情短期状态。这样能避免“请求缓存、历史仓、SDK 回源、DataFrame 标准化、响应转换”再次散落在多个调用点。
...
- 调整历史仓查询顺序、增加局部命中、引入任务队列时只改一个模块
...
`AmazingDataAdapter` 当前同时承担 SDK 加载、登录状态、BaseData/MarketData 初始化、重试、DataFrame 拼接和字段规整。这个模块的接口看起来简单，但实现仍偏宽；未来多账号负载均衡、SDK 版本适配、连接限制退避都会继续压在同一个类里。
...
- `MarketDataService`: 负责 Historical Parquet 回退 SDK、标准化输出
...
- 提取原始 SDK 查询 client
- 把 Historical Parquet 与 SDK 的数据源优先级保持在 service
